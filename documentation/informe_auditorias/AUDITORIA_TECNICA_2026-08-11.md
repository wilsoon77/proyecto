...
> **Actualización posterior:** este informe es una fotografía histórica. Después de la auditoría se cerró el recorte de alcance: no hay POS ni analítica predictiva; `PaymentMethod` queda únicamente en `EFECTIVO`; Telegram consulta solo inventario, materias primas, producción y cierres; y la caducidad admite varios días de recordatorio por producto comprado. Las decisiones posteriores y el código actual tienen prioridad sobre las recomendaciones históricas de este archivo.

> **Seguimiento:** varias recomendaciones de esta auditoría se implementaron en `20260812100000_audit_hardening`, `20260817100000_remove_cashier_role` y `20260817110000_cash_only_and_multiple_expiration_reminders`. Las observaciones que proponen POS, pagos alternativos, forecasting o notificaciones adicionales son históricas y no deben reabrirse como trabajo del alcance actual.
...
- **Aislamiento por sucursal** (`BranchScopeService`): MANAGER conserva alcance global sobre ambas sucursales; BAKER queda limitado a su sucursal asignada y CUSTOMER solo ve sus propias órdenes (`detail` con `userId`).
