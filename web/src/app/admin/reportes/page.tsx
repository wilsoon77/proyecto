    ...
    <div className="min-h-screen bg-gray-50 p-4 sm:p-6 lg:p-8">
      <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <h1 className="text-2xl font-bold text-gray-900 lg:text-3xl">Reportes y predicción de demanda</h1>
          <p className="mt-1 text-sm text-gray-500">Historial consolidado de pedidos, POS y cierre del día.</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <button onClick={syncAndReload} disabled={isRunning} className="inline-flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 disabled:opacity-50">
            <RefreshCw className={`h-4 w-4 ${isRunning ? "animate-spin" : ""}`} /> Sincronizar
          </button>
          <button onClick={runForecast} disabled={isRunning} className="inline-flex items-center gap-2 rounded-lg bg-amber-500 px-3 py-2 text-sm font-medium text-white hover:bg-amber-600 disabled:opacity-50">
      ...
      <div className="mb-6 grid grid-cols-1 gap-3 rounded-xl border border-gray-200 bg-white p-4 md:grid-cols-4">
        <label className="text-sm text-gray-600">
          Sucursal
          <select value={branchId} onChange={(event) => setBranchId(event.target.value)} disabled={!isAdmin} className="mt-1 w-full rounded-lg border border-gray-200 px-3 py-2 text-gray-900 disabled:bg-gray-100">
        ...
        <label className="text-sm text-gray-600">
          Desde
          <input type="date" value={from} onChange={(event) => setFrom(event.target.value)} className="mt-1 w-full rounded-lg border border-gray-200 px-3 py-2 text-gray-900" />
        </label>
        <label className="text-sm text-gray-600">
          Hasta
          <input type="date" value={to} onChange={(event) => setTo(event.target.value)} className="mt-1 w-full rounded-lg border border-gray-200 px-3 py-2 text-gray-900" />
        </label>
      ...
      {isLoading && !overview ? (
        <div className="rounded-xl bg-white p-10 text-center text-gray-500">Cargando analítica…</div>
      ) : (
        <>
          ...
          <div className="mb-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
            <section className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
              <div className="mb-4 flex items-center justify-between">
                <div className="flex items-center gap-2"><BarChart3 className="h-5 w-5 text-amber-600" /><h2 className="font-semibold">Demanda, producción y ventas</h2></div>
                <span className="text-xs text-gray-400">Click en un día para detalle</span>
              </div>
            ...
            <section className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
              <div className="mb-4 flex items-center justify-between">
                <div className="flex items-center gap-2"><Factory className="h-5 w-5 text-amber-600" /><h2 className="font-semibold">Predicción próximos días</h2></div>
                <span className="text-xs text-gray-400">Modelo WMA + día de semana</span>
              </div>
              {forecastSeries.length > 0 ? (
                ...
                <div className="flex h-[300px] items-center justify-center text-sm text-gray-400">Aún no hay una predicción válida.</div>
              )}
            </section>
          </div>
          ...
          <div className="mb-6 grid grid-cols-1 gap-6 lg:grid-cols-2">
            <section className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
              <div className="mb-4 flex items-center justify-between"><h2 className="font-semibold">Ventas por sucursal</h2><span className="text-xs text-gray-400">Click para navegar</span></div>
            ...
            <section className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
              <div className="mb-4 flex items-center justify-between"><h2 className="font-semibold">Productos con mayor demanda</h2><span className="text-xs text-gray-400">Click para detalle</span></div>
              <ResponsiveContainer width="100%" height={280}>
          ...
          <div className="mb-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
            <section className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm lg:col-span-2">
              <div className="mb-4 flex items-center justify-between"><h2 className="font-semibold">Producción recomendada</h2><span className="text-xs text-gray-400">Última ejecución: {forecastRuns[0] ? new Date(forecastRuns[0].generatedAt).toLocaleString("es-GT") : "sin datos"}</span></div>
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm">
                  <thead><tr className="border-b text-xs uppercase text-gray-500"><th className="pb-3">Producto</th><th className="pb-3">Fecha</th><th className="pb-3">Previsto</th><th className="pb-3">Producir</th><th className="pb-3">Confianza</th><th className="pb-3">Riesgo</th></tr></thead>
                  <tbody>
                    {forecastItems.slice(0, 15).map((item) => {
                      const risk = item.rawMaterialRisk?.status
                      return <tr key={item.id} className="border-b last:border-0 hover:bg-amber-50">
                        <td className="py-3"><button className="font-medium text-amber-700 hover:underline" onClick={() => openDrilldown("product", item.productId)}>{item.product.name}</button></td>
                        <td className="py-3 text-gray-600">{formatDay(item.forecastDate.slice(0, 10))}</td>
                        ...
                        <td className="py-3">{risk === "RISK" || risk === "NO_RECIPE" ? <span className="inline-flex items-center gap-1 text-red-600"><AlertTriangle className="h-4 w-4" /> Revisar</span> : <span className="inline-flex items-center gap-1 text-green-600"><CheckCircle2 className="h-4 w-4" /> OK</span>}</td>
                      </tr>
                    })}
                  </tbody>
                </table>
                {forecastItems.length === 0 && <div className="py-10 text-center text-sm text-gray-400">Genera una predicción para obtener recomendaciones.</div>}
              </div>
            </section>

            <section className="rounded-xl border border-gray-100 bg-white p-5 shadow-sm">
              <div className="mb-4 flex items-center gap-2"><AlertTriangle className="h-5 w-5 text-amber-600" /><h2 className="font-semibold">Calidad y fuentes</h2></div>
              <div className="space-y-4 text-sm">
                <div><p className="text-gray-500">Datos disponibles</p><p className="text-xl font-bold">{overview?.dataQuality.daysWithData || 0} / {overview?.dataQuality.totalDays || 0} días</p></div>
                <div className="h-2 rounded-full bg-gray-100"><div className="h-2 rounded-full bg-green-500" style={{ width: `${Math.round((overview?.dataQuality.coverage || 0) * 100)}%` }} /></div>
                <div className="rounded-lg bg-blue-50 p-3 text-blue-800">Pedidos/POS: <strong>{overview?.dataQuality.sources.orders || 0}</strong> uds</div>
                <div className="rounded-lg bg-amber-50 p-3 text-amber-800">Cierre del día: <strong>{overview?.dataQuality.sources.dailyCloseResidual || 0}</strong> uds residuales</div>
                <Link href="/admin/inventario" className="inline-flex items-center gap-1 text-amber-700 hover:underline">Revisar inventario <ChevronRight className="h-4 w-4" /></Link>
              </div>
            </section>
          </div>

          {overview?.lowStockProducts.length ? <section className="mb-6 rounded-xl border border-red-100 bg-white p-5 shadow-sm"><div className="mb-4 flex items-center gap-2"><AlertTriangle className="h-5 w-5 text-red-500" /><h2 className="font-semibold">Inventario bajo relacionado con la demanda</h2></div><div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">{overview.lowStockProducts.slice(0, 8).map((item) => <Link key={`${item.branchId}-${item.productId}`} href={`/admin/inventario?productId=${item.productId}`} className="rounded-lg border border-gray-100 p-3 hover:border-amber-300"><p className="font-medium">{item.productName}</p><p className="text-xs text-gray-500">{item.branchName}</p><p className="mt-1 text-red-600">{item.available} uds disponibles</p></Link>)}</div></section> : null}

          {drilldown && <section className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm"><div className="mb-4 flex items-center justify-between"><div><div className="mb-1 flex items-center gap-2"><CalendarDays className="h-5 w-5 text-amber-600" /><h2 className="font-semibold">Detalle: {drilldown.level === "product" ? "producto" : drilldown.level === "branch" ? "sucursal" : "día"}</h2></div><p className="text-xs text-gray-500">{drilldown.range.from} a {drilldown.range.to} · {drilldown.meta.total} registros</p></div><button onClick={() => setDrilldown(null)} className="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-900"><ArrowLeft className="h-4 w-4" /> Cerrar</button></div><div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead><tr className="border-b text-xs uppercase text-gray-500"><th className="pb-3">Nivel</th><th className="pb-3">Demanda</th><th className="pb-3">Pedidos/POS</th><th className="pb-3">Cierre</th><th className="pb-3">Producción</th><th className="pb-3">Merma</th><th className="pb-3">Fuentes</th></tr></thead><tbody>{drilldown.data.map((row, index) => <tr key={`${row.key}-${index}`} className="border-b last:border-0"><td className="py-3 font-medium">{row.productName || row.branchName || row.date || row.businessDate || row.reference || "—"}</td><td className="py-3">{row.demandQty ?? row.quantity ?? 0}</td><td className="py-3">{row.orderQty ?? "—"}</td><td className="py-3">{row.dailyCloseQty ?? "—"}</td><td className="py-3">{row.productionQty ?? "—"}</td><td className="py-3">{row.wasteQty ?? "—"}</td><td className="py-3">{row.source ? <Link href={row.href || "#"} className="text-amber-700 hover:underline">{row.source}</Link> : row.productId ? <button onClick={() => openDrilldown("source", row.productId)} className="text-amber-700 hover:underline">Ver fuentes</button> : "—"}</td></tr>)}</tbody></table></div></section>}
        </>
      )}
    </div>
  )
