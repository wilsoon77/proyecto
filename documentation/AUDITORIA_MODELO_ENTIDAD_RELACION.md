...
> **El modelo es correcto y está bien normalizado. Las 33 tablas no son "muchas": son el resultado de que este sistema no es una tienda en línea de panadería, sino un ERP operativo pequeño (producción por amasijos + inventario multi-sucursal con lotes y caducidad + cierres de caja + POS + notificaciones multicanal + auditoría).**
...
**Observación:** `targetRoles` es el único que podría necesitar consulta inversa ("¿qué notificaciones recibe un CASHIER?"). Si esa consulta llega a ser frecuente, se normalizaría con una tabla puente `NotificationConfigRole`. Hoy no se justifica: el volumen es de decenas de filas y se resuelve en memoria.
