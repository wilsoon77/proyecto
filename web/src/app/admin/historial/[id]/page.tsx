    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-3xl mx-auto">
      ...
      <div className="bg-card rounded-xl shadow-sm border border-border p-5 sm:p-6 mb-6">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-4">
          ...
          <span className="inline-flex items-center gap-1.5 px-3 py-1 text-sm font-medium rounded-full bg-muted text-foreground">
        ...
        <div className="bg-card rounded-xl shadow-sm border border-border p-5 sm:p-6 mb-6">
          ...
          <dl className="divide-y divide-gray-100">
            {summary.items.map((item, idx) => (
              ...
              >
                <dt className="text-sm font-medium text-muted-foreground sm:w-44 sm:flex-shrink-0">
                  {item.label}
                </dt>
                  ...
                  {item.label === "Campos modificados" ? (
                    <div className="flex flex-wrap gap-1.5">
                      {summary.changedFields?.map((field, i) => (
                        <span
                          key={i}
                          className="inline-flex px-2 py-0.5 text-xs font-medium rounded bg-chart-3/10 text-chart-3"
                        >
                          {field}
                        </span>
                      ))}
      ...
      {/* Metadata Card */}
      <div className="bg-card rounded-xl shadow-sm border border-border p-5 sm:p-6 mb-6">
        <h2 className="text-base font-semibold text-foreground mb-4">Información del Registro</h2>
        ...
        <dl className="space-y-4">
          ...
          {/* Date */}
          <div className="flex items-start gap-3">
            <Clock className="h-5 w-5 text-muted-foreground/60 mt-0.5 flex-shrink-0" />
            <div>
              <dt className="text-xs font-medium text-muted-foreground uppercase">Fecha y Hora</dt>
              <dd className="text-sm text-foreground">{formatDate(log.createdAt)}</dd>
            </div>
          </div>
          ...
          {log.userAgent && (
            <div className="flex items-start gap-3">
              <Monitor className="h-5 w-5 text-muted-foreground/60 mt-0.5 flex-shrink-0" />
              <div>
                <dt className="text-xs font-medium text-muted-foreground uppercase">Navegador</dt>
                <dd className="text-sm text-muted-foreground break-all">{log.userAgent}</dd>
              </div>
            </div>
          )}
        </dl>
      ...
      {log.details && (
        <details className="bg-card rounded-xl shadow-sm border border-border p-5 sm:p-6">
          ...
          <pre className="mt-3 p-3 bg-cream rounded-lg text-xs text-foreground overflow-x-auto whitespace-pre-wrap break-words">
